...
def update_relationships(G, want_main, want_casual, ex_list_main, ex_list_casual, wait_time_main, wait_time_main_list, wait_time_casual, wait_time_casual_list):      
    removed_main = []
    removed_casual = []
    ...
    if len(want_main) > 0:
        ...
        while len(want_main_copy)>0:
            ...
            if len(poss)>0:
                ...
                # if new relationship is formed, add to the tracker how long the wait time was
                wait_time_main_list.append(wait_time_main[maxnode])
                wait_time_main_list.append(wait_time_main[minnode])
                
                # reset each node's wait time to 0
                wait_time_main[maxnode] = 0
                wait_time_main[minnode] = 0
                ...
                # add to the node's wait time
                wait_time_main[node] += 1
                continue
        ...
        while len(want_casual_copy)>0:
            ...
            if len(poss)>0:
                ...
                new_edge +=1
                 ...
                 # if new relationship is formed, add to the tracker how long the wait time was
                wait_time_casual_list.append(wait_time_casual[maxnode])
                wait_time_casual_list.append(wait_time_casual[minnode])
                
                # reset each node's wait time to 0
                wait_time_casual[maxnode] = 0
                wait_time_casual[minnode] = 0
                ...
                # add to the node's wait time
                wait_time_casual[node] += 1
                continue
        # Keep the skipped nodes on the list of who wants a casual partnership
        want_casual = skipped        
                
    return(G, want_main, want_casual, ex_list_main, ex_list_casual, wait_time_main, wait_time_main_list, wait_time_casual, wait_time_casual_list)
        ...
        for n in neighbors:
            ...
            # Get the node order (min,max) to get the right edge
                ...
                if ((G.edges[min_node,max_node,key]['rel_type']=='Main' and random.random() < pm) 
                    ...
                    contact = 1
                    total_contacts[min_node] += 1
                    total_contacts[max_node] += 1
...
def simulate(N, n_initial, p_infect, steps, intervention_start, behavior_change, 
    ...
    for i in range(1,steps):   
        ...
        G, want_main, want_casual, ex_list_main, ex_list_casual, wait_time_main, wait_time_main_list, wait_time_casual, wait_time_casual_list = update_relationships(G, want_main, want_casual, ex_list_main, ex_list_casual, wait_time_main, wait_time_main_list, wait_time_casual, wait_time_casual_list)
        ...
        print('day: ', i)
        print('num infected: ', np.sum(status[:,0]))
        
        
    return(number_E, number_I, number_R, infection_tracker, wait_time_main_list, wait_time_casual_list, num_contacts, main_degseq, onetime_degseq, rel_activity, activity_strat)
